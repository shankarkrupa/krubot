...
Execute one shell command on the computer running this client and return its
captured result to the language model. The command runs with the operating
system's normal shell: `/bin/sh` on Linux and Android Termux, and the default
`COMSPEC` shell on Windows. Use this tool for local inspection, file work,
development commands, and other tasks that the user has requested.
...
- Linux or Termux: `pwd`, `ls -la`, `find . -maxdepth 2 -type f`, and `uname -a`.
- Windows: `Get-Location`, `Get-ChildItem -Force`, `Get-ChildItem -Recurse -File`, and `Get-ComputerInfo`.
- Use the shell syntax native to the detected platform. Do not assume Bash is
  installed on Windows or PowerShell is installed in Termux.
...
### Run a project test from a directory

```json
{"command":"python -m pytest","cwd":"/path/to/project","timeout_seconds":120}
```

### Check a Windows directory

```json
{"command":"Get-ChildItem -Force","cwd":"C:\\path\\to\\project"}
```
